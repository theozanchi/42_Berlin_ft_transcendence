...
class PongConsumer(WebsocketConsumer):
    ...
    def connect(self):
        ...
        self.send(
            text_data=json.dumps({"type": "player_identity", "player_id": client_id})
        )
    ...
    def update_ball(self):
        ...
        if (
            self.game_state["ball"]["x"] <= -half_cube_size
            or self.game_state["ball"]["x"] >= half_cube_size
        ):
        ...
        if (
            self.game_state["ball"]["y"] <= -half_cube_size
            or self.game_state["ball"]["y"] >= half_cube_size
        ):
        ...
        if (
            self.game_state["ball"]["z"] <= -half_cube_size
            or self.game_state["ball"]["z"] >= half_cube_size
        ):
    ...
    def check_collision(self):
        ...
        if (
            self.game_state["ballSpeed"]["x"] == 0
            and self.game_state["ballSpeed"]["y"] == 0
            and self.game_state["ballSpeed"]["z"] == 0
        ):
            return False
        ...
        player_box = self.create_bounding_box(
            self.game_state["player1"], self.game_state["player1"]
        )
        ai_player_box = self.create_bounding_box(
            self.game_state["player2"], self.game_state["player2"]
        )

        # Check if the ball is colliding with the player or AI player
        if (
            self.game_state["playerTurn"] and self.intersects_box(ball_box, player_box)
        ) or (
            not self.game_state["playerTurn"]
            and self.intersects_box(ball_box, ai_player_box)
        ):

            # Place collision marker at the intersection point for debugging
            self.collision_marker_position = ball_position.copy()
            logging.info(
                f'Collision Detected: {"Player" if self.game_state["playerTurn"] else "AI Player"}'
            )
            ...
            paddle_scale = {
                ...
                "y": 1,
                "z": 1,
            }  # Assuming scale is 1 for simplicity
            ...
            new_ball_speed = {
                "x": self.game_state["ballSpeed"]["x"]
                + relative_collision_point["x"] * 0.5,
                "y": self.game_state["ballSpeed"]["y"]
                + relative_collision_point["y"] * 0.5,
                "z": -self.game_state["ballSpeed"]["z"]
            ...
            speed_increment = (
                0.02  # Adjust this value to control the speed increase rate
            )
            new_ball_speed = {
            ...
            self.game_state["ball"] = {
                ...
                + self.game_state["ballSpeed"]["x"] * 0.1,
                "y": self.game_state["ball"]["y"]
                + self.game_state["ballSpeed"]["y"] * 0.1,
                "z": self.game_state["ball"]["z"]
                + self.game_state["ballSpeed"]["z"] * 0.1,
            }
    ...
    def clone_and_add(self, position, direction, offset_distance):
        return {
            k: position[k] + direction[k] * offset_distance for k in position.keys()
        }
    ...
    def reset_ball(self):
        ...
        if not self.game_state["playerTurn"]:
            ball_start_position = {
                k: self.game_state["player1"][k] + direction[k] * offset_distance
                for k in direction
            }
        else:
            ball_start_position = {
                k: self.game_state["player2"][k] + direction[k] * offset_distance
                for k in direction
            }
    ...
    def calculate_direction(
        self, player_turn, current_face, current_face2, aiming_angle
    ):
    ...
    def send_game_state(self):
        for self.client, client in PongConsumer.clients.items():
            game_state_message = {"type": "game_state"}
    ...
    def game_update(self, data):
        ...
        self.game_state["current_face"] = data.get(
            "current_face", self.game_state["current_face"]
        )
        self.game_state["current_face2"] = data.get(
            "current_face2", self.game_state["current_face2"]
        )
        self.game_state["aiming_angle"] = data.get(
            "aiming_angle", self.game_state["aiming_angle"]
        )
        self.game_state["reset_ball"] = data.get(
            "reset_ball", self.game_state["reset_ball"]
        )
